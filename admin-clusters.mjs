...
// Collision layouts use one fixed map scale per display layer.  Re-projecting
// against the live zoom made the same administrative bubble choose a different
// 0-32 px detour while the user zoomed inside one layer, which looked like the
// province nodes were jumping around.
const COLLISION_REFERENCE_ZOOMS = Object.freeze({
  province: 4,
  prefecture: 6,
  county: 8,
  spread: 9,
...
/**
 * Project a marker at the fixed collision scale for its display layer.
 *
 * Only relative screen-space distance is needed by `resolveAdminCollisions`,
 * so a world-pixel Web Mercator point is sufficient and stays independent of
 * the current map center and fractional zoom.  Geographic marker coordinates
 * remain untouched.
 */
export function projectStableCollisionPoint(lnglat, mode = 'province') {
  const point = readPoint(lnglat);
  if (!point) return null;

  const referenceZoom = COLLISION_REFERENCE_ZOOMS[mode] ?? COLLISION_REFERENCE_ZOOMS.cinema;
  const worldSize = 256 * (2 ** referenceZoom);
