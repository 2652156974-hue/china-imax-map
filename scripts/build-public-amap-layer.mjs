...
export function buildPublicAmapLayer({
  ...
  const rows = input.records.map((record) => toDecision(record, context));
  ...
  const output = {
    ...
    summary: {
      ...
      canonicalIdentityExact: accepted.filter((record) => record.identityConfidence === 'high').length,
      canonicalLocationOnly: accepted.filter((record) => record.identityConfidence === 'medium').length,
  ...
  writeJson(qualityFile, {
    ...
    canonicalIdentityExact: output.summary.canonicalIdentityExact,
    canonicalLocationOnly: output.summary.canonicalLocationOnly,
...
function toDecision(record, administrativeContext) {
  ...
  return {
    ...
    administrative: bindAdministrativeRecord(record, administrativeContext)
  };
}
