// Shared, platform-neutral marker validation and response shaping.
...
const ADMINISTRATIVE_FIELDS = Object.freeze([
  'provinceName',
  'provinceCode',
  'prefectureName',
  'prefectureCode',
...
export function validateRuntimeMarkerLayer(layer) {
  ...
  for (const record of layer.records) {
    ...
    rows.add(record.sourceRow);
    if (record.administrative !== undefined) validateAdministrative(record.administrative);
  }
  return layer;
}
...
export function selectMarkers(layerOrConfig, requestedRows) {
  ...
  const records = layerOrConfig?.markerMap instanceof Map
...
function minimalAdministrative(value) {
  const administrative = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  return Object.fromEntries(ADMINISTRATIVE_FIELDS.map((field) => [field, administrative[field] ?? null]));
}

function validateAdministrative(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('Runtime marker administrative metadata is invalid.');
  for (const field of ADMINISTRATIVE_FIELDS) {
    const fieldValue = value[field];
    if (fieldValue !== null && fieldValue !== undefined && typeof fieldValue !== 'string') {
      throw new Error(`Runtime marker administrative field ${field} must be text or null.`);
    }
  }
}
