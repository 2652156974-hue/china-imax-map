...
export function buildCloudflarePublic() {
  ...
  const files = collectFiles(DIST_ROOT)
    .map((file) => path.relative(DIST_ROOT, file).replaceAll(path.sep, '/'))
    .sort();
  if (files.some((file) => /(?:data\/local|data\/raw|provider-cache|rawCandidates|rankedCandidates|coordinate|marker).*(?:\.json|\.csv|\.geojson)$/i.test(file))) {
    throw new Error('Cloudflare static output contains a private or coordinate artifact.');
  }
  const outputText = files
    .filter((file) => /\.(?:html|mjs|js|css|json|txt)$/i.test(file))
    .map((file) => fs.readFileSync(path.join(DIST_ROOT, file), 'utf8'))
    .join('\n');
  if (/AMAP_JS_SECURITY_CODE|securityJsCode|rawCandidates|rankedCandidates/i.test(outputText)) {
    throw new Error('Cloudflare static output contains private candidates or credentials.');
  }
...
function validatePublicDataset(dataset) {
  ...
  if (dataset.policy?.staticBulkCoordinateArtifacts !== 0 || dataset.policy?.amapCoordinatesIncluded !== false) {
    throw new Error('Cloudflare static build requires the public coordinate boundary.');
  }
  const serialized = JSON.stringify(dataset);
  if (/providerLat|providerLng|rawCandidates|rankedCandidates|AMAP_JS_SECURITY_CODE/i.test(serialized)) {
  ...
  for (const record of dataset.records) {
    if (record.location?.lat !== null || record.location?.lng !== null) {
      throw new Error(`Public record ${record.id} contains static coordinates.`);
    }
  }
}

function assertSafeOutput(outputRoot) {
  if (path.resolve(outputRoot) !== path.join(ROOT, 'dist-public')) {
