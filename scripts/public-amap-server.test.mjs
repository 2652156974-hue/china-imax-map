...
test('public marker service returns only requested minimal AMap GCJ-02 records', async (t) => {
  ...
  assert.equal(typeof selected[0].administrative, 'object');
  assert.equal(selected[0].administrative.provinceName, locatedRow.administrative?.provinceName ?? null);
  assert.equal(Object.hasOwn(selected[0].administrative, 'rawCandidates'), false);
  assert.equal(Object.hasOwn(selected[0].administrative, 'address'), false);
  assert.equal(Object.hasOwn(selected[0].administrative, 'cache'), false);
  ...
  assert.deepEqual(markerBody.records[0].administrative, selected[0].administrative);
